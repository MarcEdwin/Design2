...
\section{Wednesday, March 22, 2017}
Tasks for next week:
\begin{enumerate}
\item Do the same thing we did this past week on the Arduino.
\item Change the broadcast radius and see what the effect is on the transmission distance.
\item For a given broadcast radius, check the checksum errors for a few different distances.
\item Check if ACKs are sent out only when the checksum is good or in general when a fram is received.
\end{enumerate}
